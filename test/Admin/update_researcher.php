...
$get_email=mysqli_query($con,"SELECT * FROM Researcher WHERE id='$id'");
while($get=mysqli_fetch_array($get_email))
{
	$org_email=$get['email'];
	$pass=$get['passwd'];
}
...
{
	if($insert=mysqli_query($con,"UPDATE `Researcher` SET `fname`='$fname',`lname`='$lname',`email`='$email',`workshop`='$workshop' WHERE `id`='$id'"))
	{
		session_start();
...
// $frmmessage	=	$_POST['frmmessage'];
	 ...
	 $message = "Updated Login details For SUS Form:<br>Name: $fname $lname<br>Username: $email<br>Password: $pass<br>Workshop: $wname<br>Link: www.e-gyaan.in/IITB/SUS/Researcher/<br>SUS Form: www.e-gyaan.in/IITB/SUS/forms/$sys";
